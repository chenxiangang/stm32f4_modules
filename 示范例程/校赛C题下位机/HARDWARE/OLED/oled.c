...
void OLED_ShowFloat(u8 x, u8 y, float val)
{
	if (val < 0)
	{
		...
		OLED_ShowChar(x, y, ' ', 12, 1);
	OLED_ShowNum(x + 8, y, val, 4, 12);
	OLED_ShowChar(x + 32, y, '.', 12, 1);
	OLED_ShowNum(x + 38, y, (val - (u8)val) * 100, 2, 12);
}

void OLED_ShowMPU(int tmode, int omode, u16 set)
{
	//OLED_ShowString(36, 0, Pro_Name, 16);
	OLED_ShowString(5, 0, "TMODE :", 12);
	OLED_ShowString(5, 18, "OMODE  :", 12);
	OLED_ShowString(5, 32, "SET   :", 12);
	//OLED_ShowString(5, 46, "PWM   :", 12);
	if (tmode)
	{
		OLED_ShowString(56, 0, "CAN", 12);
	}
	else
	{
		OLED_ShowString(56, 0, "PWM", 12);
	}
	if (omode)
	{
		OLED_ShowString(56, 18, "angle", 12);
	}
	else
	{
		OLED_ShowString(56, 18, "speed", 12);
	}
	OLED_ShowNum(56, 32, set, 4, 12);
	//OLED_ShowFloat(56, 0, angel);
	//OLED_ShowFloat(56, 18, roll);
	//OLED_ShowFloat(56, 32, pitch);
	//OLED_ShowFloat(56, 46, yaw);
