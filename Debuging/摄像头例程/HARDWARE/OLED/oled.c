...
void Camera_OLED(u8 x,u8 y)
{
	...
	//超过OLED范围,横没超,竖超
	if(CAMERA_H>64 && CAMERA_W<=128 )
	{
		ri = CAMERA_H/(float)64; //缩放比例
		rj = 1;
		imax = 64;
		jmax = CAMERA_W;
	}
	...
	if(CAMERA_W>128 && CAMERA_H>64)
	{
		ri = CAMERA_H/(float)64; 
		rj = CAMERA_W/(float)128; 
		imax = 64;
		jmax = 128;
	}
    ...
    {
		h = (int)((ri * i)+0.5);   //四舍五入
        for(j = 0; j < jmax; j++) //CAMERA_W>CAMERA_H
        {
			w = (int)((rj * j)+0.5);
            ...
            if(d<threshold.real)
            {
                OLED_DrawPoint(x+j,y+i,1);//填充 1  亮
            }
            else
            {
                OLED_DrawPoint(x+j,y+i,0); //0  暗
            }
