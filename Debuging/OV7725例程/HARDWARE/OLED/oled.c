...
void OLED_ShowFontHZ(u8 x, u8 y, u8 pos, u8 size, u8 mode)
{
	...
	{
		if (size == 12)
			temp = FontHzk[pos][t]; //调用1206字体
		else if (size == 16)
			temp = FontHzk[pos][t]; //调用1608字体
		else if (size == 24)
			temp = FontHzk[pos][t]; //调用2412字体
		else
			return; //没有的字库
		for (t1 = 0; t1 < 8; t1++)
		{
...
void OLED_ShowFloat(u8 x, u8 y, float val)
{
	if (val < 0)
	{
		...
		OLED_ShowChar(x, y, ' ', 12, 1);
	OLED_ShowNum(x + 8, y, val, 3, 12);
	OLED_ShowChar(x + 26, y, '.', 12, 1);
	OLED_ShowNum(x + 30, y, (val - (u8)val) * 100, 2, 12);
}
...
void OLED_Show(u8 x, u8 y)
{
	 ...
	 * 缩放
	*********/
	if (CAMERA_H > 64 && CAMERA_W * 8 <= 128)
	{
		ri = CAMERA_H / (double)64; //缩放比例
		rj = 1;
		imax = 64;
		jmax = CAMERA_W;
	}
	if (CAMERA_W * 8 > 128 && CAMERA_H <= 64)
	{
		rj = (CAMERA_W * 8) / (double)128; //缩放比例
		ri = 1;
		imax = CAMERA_H;
		jmax = 128;
	}
	if (CAMERA_W > 128 && CAMERA_H > 64)
	{
		ri = CAMERA_H / (double)64;
		rj = (CAMERA_W * 8) / (double)128;
		imax = 64;
		jmax = 128;
	}
	for (i = 0; i < imax; i++)
	{
		h = (int)((ri * i) + 0.5);		 //四舍五入
		for (j = 0; j < jmax; j = j + 8) //CAMERA_W>CAMERA_H
		{
			w = (int)((rj * j) + 0.5);
			d = Image[h][w];
			for (k = 0; k < 8; k++)
				OLED_DrawPoint(x + j, y + i, d & (1 << (7 - k)));
		}
	}
	OLED_Refresh_Gram();
}
