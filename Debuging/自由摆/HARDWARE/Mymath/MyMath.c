...
//输入范围：[-1,1]
//输出范围：[-PI/2*A2R,PI/2*A2R]
float my_asin(float r)
{
	return my_atan( my_sqrt( 1 - r*r  ), r );
}
...
{
	return my_atan( r , my_sqrt( 1 - r*r  ) );
}
...
//----------------------------  第一类完全曲面积分  ----------------------------//
double superposition_function(double t,double m)
{
    double s;
    s=1/my_sqrt(1-m*my_sin(t)*my_sin(t));     //sqrt(4-t*t);
    return (s);
}

//积分下限，积分上限，分割数量，参数m
double superposition_T(double x,double y,int z,double m)
{
    double h,Tn;
    int i;
    h=(y-x)/z;
    Tn=(superposition_function(x,m)+superposition_function(y,m))/2;
    for(i=1; i<z; i++) Tn=Tn+superposition_function(x+i*h,m);
    Tn=Tn*h;
    return (Tn);
}  

/* 定积分函数，用Simpson公式计算，非常精准 */
//分割次数,参数m
double superposition_Km(int n,double m)
{
	double s;
	
	s=(4*superposition_T(0,_PI/2,2*n,m)-superposition_T(0,_PI/2,n,m))/3;
...
//简谐运动方程，初相位为0
//单位均为国际单位
double simple(double a,double omega,double t)
{
    return ( a * my_cos( omega * t ) );
}
...
//alpha:摆角
//length:摆长，国际单位（米）
double T_calculate_big(double alpha,double length)
{
    return ( 4 * my_sqrt(length/9.8)* superposition_Km( 10,my_pow( my_sin(alpha/2),2 ) ) );
}
