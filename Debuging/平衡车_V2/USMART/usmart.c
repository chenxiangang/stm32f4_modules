...
void usmart_reset_runtime(void)
{
    ...
    TIM_ClearFlag(TIM9, TIM_FLAG_Update); //清除中断标志位
    TIM_SetAutoreload(TIM9, 0XFFFF); //将重装载值设置到最大
    TIM_SetCounter(TIM9, 0); //清空定时器的CNT
...
{
    if (TIM_GetFlagStatus(TIM9, TIM_FLAG_Update) == SET) //在运行期间,产生了定时器溢出
    {
        usmart_dev.runtime += 0XFFFF;
    }
    usmart_dev.runtime += TIM_GetCounter(TIM9);
...
void TIM9_IRQHandler(void)
{
    if (TIM_GetITStatus(TIM9, TIM_IT_Update) == SET) //溢出中断
    {
        ...
        TIM_SetCounter(TIM9, 0); //清空定时器的CNT
        TIM_SetAutoreload(TIM9, 100); //恢复原来的设置
    }
    TIM_ClearITPendingBit(TIM9, TIM_IT_Update); //清除中断标志位
}
//使能定时器4,使能中断.
void Timer9_Init(u16 arr, u16 psc)
{
    TIM_TimeBaseInitTypeDef TIM_TimeBaseInitStructure;

    RCC_APB1PeriphClockCmd(RCC_APB2Periph_TIM9, ENABLE); ///使能TIM1时钟
    ...
    TIM_TimeBaseInit(TIM9, &TIM_TimeBaseInitStructure); //初始化定时器4

    TIM_ITConfig(TIM9, TIM_IT_Update, ENABLE); //允许定时器4更新中断
    TIM_Cmd(TIM9, ENABLE); //使能定时器4
}
#endif
////////////////////////////////////////////////////////////////////////////////////////
//初始化串口控制器
...
{
#if USMART_ENTIMX_SCAN == 1
    Timer9_Init(1000, (u32)sysclk * 100 - 1); //分频,时钟为10K ,100ms中断一次,注意,计数频率必须为10Khz,以和runtime单位(0.1ms)同步.
#endif
